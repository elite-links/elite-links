...
app.post("/api/login", async (req, res, next) => {
  try {
    ...
    if (!user.paid) return res.status(403).json({ error: "Payment required" });
...
/* ---- ADMIN PANEL ---- */
app.get("/api/admin", requireAuth, requireAdmin, (_, res) => {
...
/* ---- CREATE PROFILE ---- */
app.post("/create-profile", requireAuth, async (req, res, next) => {
  try {
    await Profile.create({ userId: req.user.id, ...req.body });
...
/* ---- DASHBOARD PAGE ---- */
app.get("/dashboard.html", requireAuth, (_, res) => {
